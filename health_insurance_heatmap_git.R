...
# use data
mine.data <- read.csv(file = "r_HIproduct_data/heatmap_HI_products_statewise.csv") #use tab to fill complete the word
...
# creating final heat map plot
library(cowplot)
mine.heatmap12 <- ggplot(data = mine.long, mapping = aes(x = Scheme.Name,
                                                         y = State,
                                                         fill = Percentages)) +
...
mine.heatmap12 # show the created plot

ggdraw(add_sub(mine.heatmap12,"Source: Author's Calcutions from NFHS-4 data", size = 10, fontface="italic")) 
